...
<xsl:template name="location_group">
			<table>
				  ...
				  <xsl:variable name="addresses_added" select="count(property[@prop_group_id='1' and @ron &gt; '0'])" />
							...
							<xsl:if test="@epi = 0">
								...
								<a href="#" >
									<xsl:attribute name="onclick"> 
										...
										regexstr = regexstr.substr(0,regexstr.length - 2);
										 if (document.getElementById('laptop').style.display == 'none')
										 {
									 ...
									 <xsl:when test="@prop_name = 'State'" >
										<xsl:if test="@epi &gt; 0">
											...
											<xsl:call-template name="print_lookup_value_in_input_field" >
												<xsl:with-param name="lkp_tbl_path" select="/myedbroot/lookup_table_states/state[@state_id=$var_value]" />
											</xsl:call-template>
										</xsl:if>
										<xsl:if test="@epi = 0">
