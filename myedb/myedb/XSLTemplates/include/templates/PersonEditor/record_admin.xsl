		...
		<td valign="top" width="34%" style="background-image:url(/images/green.jpg);">
			...
			<tr><td>Fields required *</td>
				 ...
				 <xsl:apply-templates select="property[@prop_name='User ID']" >
				 			<xsl:with-param name="field_title" select="'* Admin ID (Email Address)'" />
				</xsl:apply-templates>
				 <xsl:if  test="$eid = 0">
				 </xsl:if>
				...
				<xsl:apply-templates select="property[@prop_id=55]" mode="hidden_input_field">
 				...
 				<xsl:apply-templates select="property[@prop_id=25]"><!--@prop_name='User Authorization Level'-->
				 			<xsl:with-param name="field_title" select="'* Admin Security Level (1-4)'" />
				</xsl:apply-templates>
